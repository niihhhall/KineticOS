...
import { Monitor, Globe, Share2, Star, MoreHorizontal, ExternalLink, Lock, MousePointer2, ShieldCheck, Play, Check, Zap, X, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className={`overflow-hidden relative flex flex-col justify-center items-center ${className}`}>
      <motion.div
        ...
        // OPTIMIZATION: Centered origin for perfect mobile alignment
        className="origin-center block will-change-transform text-center mx-auto"
      >
        {children}
      </motion.div>
    </div>
...
export const Hero: React.FC<HeroProps> = ({ onReady, openWaitlist }) => {
  ...
  return (
    <section 
      ...
      // Uses min-h-[100dvh] for mobile alignment precision
      className="relative min-h-[100dvh] flex flex-col justify-center items-center overflow-hidden bg-brand-bg px-6 md:px-12 lg:px-24 pt-24 pb-16 perspective-1000"
    >
                             ...
                             <div className="relative bg-white/60 backdrop-blur-2xl border border-white/60 p-2 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] flex flex-col md:flex-row gap-2">
                                ...
                                <div className="bg-white/50 rounded-[1.5rem] p-6 md:p-8 flex-1 flex flex-col justify-center items-center border border-black/[0.03] shadow-inner relative overflow-hidden">
                                   ...
                                   <Button 
                                      ...
                                      className="w-full text-lg px-8 py-4 shadow-[0_8px_30px_-5px_rgba(255,117,31,0.4)] hover:shadow-[0_12px_40px_-5px_rgba(255,117,31,0.6)] flex items-center justify-center gap-2 group"
                                   >
                                      Join the Waitlist
                                   </Button>
                                   <div className="mt-4 flex flex-wrap items-center justify-center gap-3 text-[10px] font-bold uppercase tracking-widest">
                                ...
                                {/* Urgency Zone (Right) */}
                                <div className="bg-gray-100/50 rounded-[1.5rem] p-6 md:px-10 flex flex-col justify-center items-center border border-black/[0.03] w-full md:w-auto md:min-w-[260px] relative">
                                   <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-3">
